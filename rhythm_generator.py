...
def nsr(
	...
	for h in range(subplots):
		...
		while beat_start < sizer:
			...
			y[start:min(start + y_p.size,sizer)] = np.add(evenly_spaced_y(x_p[:min(x_p.size,sizer - start)],y_p[:min(x_p.size,sizer - start)]),
			...
			if overlap > 0 and beat_counter > 0:
				y_label[start : start + overlap] = 7.

			if start + y_p.size < sizer:
				y_label[start + y_p.size : min(start + pr_interval, sizer)] = 2.
			...
			y[start : min(start + y_qrs.size, sizer)] = evenly_spaced_y(x_qrs[: min(x_qrs.size, sizer - start)],\
			...
			if include_points:
				for pk in range(len(wave_peak_list)):
					if wave_peak_list[pk] != -1 and start + wave_peak_list[pk] < sizer:
						y_label[start + wave_peak_list[pk]] = 10. + pk
			...
			if (st_length > 0):
				...
				y_label[start:min(start + x_st.size, sizer)] = 4.
			...
			y[start:min(start + x_t.size,sizer)] = evenly_spaced_y(x_t[: min(x_t.size, sizer - start)],\
				y_t[: min(x_t.size, sizer - start)])
			...
			if st_length > 5 and min(start + x_t.size,sizer) - start > 5:
				...
				for t_value in range(start, min(start + x_t.size, sizer), 1):
					if abs(st_grad - grad[t_value - start]) < t_grad:
						...
						y_label[t_value : min(start + x_t.size,sizer)] = 5.
						break
			else:
				y_label[start:min(start + x_t.size,sizer)] = 5.
			...
			beat_start += gap
			beat_counter += 1
			y_label[min(start + x_t.size, sizer) : min(sizer, beat_start)] = 6.
			...
			if x_t.size > 1 and start + x_t.size < sizer:
				if include_points:
					...
					if y[start + max_slope_x_coordinate] != 0 and grad[max_slope_x_coordinate] != 0:
						...
						qt_end = int(max_slope_x_coordinate + (y[start + max_slope_x_coordinate] / -grad[max_slope_x_coordinate]))
						if start + qt_end < sizer and qt_end > 0:
							y_label[start + qt_end] = 15.
							qt_end_set = True
				y_label[start + x_t.size] = 16.

		y = smooth_and_noise(y, universal_noise_multiplier = universal_noise_multiplier, impedance = impedance)
		X[h,] = x
		Y[h,] = y
		Y_label[h,] = y_label
...
def af(
	...
	for h in range(subplots):
		...
		if beat_length > int(gap - (variability * gap)) + qrs_duration + st_length + t_length:
			print('Error, heart rate too high for beat length')
			exit()
				...
				if include_points:
					...
					if y[start + max_slope_x_coordinate] != 0 and grad[max_slope_x_coordinate] != 0:
						...
						if start + qt_end < sizer and qt_end > 0:
							y_label[start + qt_end] = 15.
							qt_end_set = True
				y_label[start + x_t.size] = 16. # 25.

		y = smooth_and_noise(y, rhythm = 'af', universal_noise_multiplier = universal_noise_multiplier, impedance = impedance)
		X[h,] = x
		Y[h,] = y
		Y_label[h,] = y_label
